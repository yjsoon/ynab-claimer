...
Use these defaults for this repo unless the user overrides them:

- Lovable Labs claims: memo `Lovable AI and Cloud for IMDA Vibe`, Xero category `Cost of Sales (320)`.
- Sheng Siong or Giant snack claims: memo `Snacks for Swift Accelerator students`, Xero category `Cost of Sales (320)`, and use `INPUTY24:Standard-Rated Purchases` when the receipt explicitly shows GST.
- Grab, Gojek, and other taxi claims: Xero category `Local Public Transport (incl Taxi) (451)` and tax code `NRINPUT:Purchases from Non-GST Registered Suppliers`.
